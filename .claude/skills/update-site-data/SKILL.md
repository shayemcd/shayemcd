...
description: Updates the data/*.json content files that drive the site — publications, working papers, media/press, education, ongoing projects, news, talks, software, teaching, and the profile/bio. Use when adding a paper or preprint, converting a working paper to published, or adding/editing news, media, talk, software, project, teaching, education, or profile entries.
...
This site was adapted from the [cc-academic-website](https://github.com/mr-devs/cc-academic-website) template and no longer matches it exactly (single page, no `docs/publications/` per-paper directories, and two extra sections — `media` and `education` — that the upstream template doesn't have). Before any edit:
...
| Media/press coverage, talk, software, ongoing project, teaching entry, education entry, profile/bio change, or a direct edit (fixing a typo, updating a URL) | Handle inline — see below |

...
Add a news item to `data/news.json`. See [schemas.md](schemas.md) for the schema, the `htmltext` HTML conventions, and the per-type text patterns. If no details were provided, ask the user what to add. Check the live `data/news.json` first — match its actual structure and style. This file is currently empty (the News section is hidden until it has entries) — adding the first item will make the section appear.
